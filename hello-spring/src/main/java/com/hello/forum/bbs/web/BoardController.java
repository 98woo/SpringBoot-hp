...
public class BoardController {
	 ...
	 * 사용하는 방법 > 파라미터 처리가 매우 편하다!!! 4. @PathVariable
	 * 
	 * @return
	 */
...
//			@RequestParam String content
			...
			// @Valid
			/*
			 * @NotEmpty, @Email, @Size, @Min 등 을 검사하도록 지시한다.
			 * 
			 * @Valid 가 없으면 어노테이션을 쓰더라도 유효성 검사가 동작하지 않는다.
			 */ BoardVO boardVO,
			/*
			 * @Valid 에 의해 실행된 파라미터 검사 (@Not Empty, Email, Min 등 )의 결과. BindingResult에 저장된다.
			 */
//			BindingResult bindingResult,
		...
		/*
		 * Servlet Like HttpServletRequest를 이용. - Interceptor에서 이용 - Filter에서 이용.
		 */
...
//			model.addAttribute("boardVO", boardVO);		// key value boardVO 라는 이름으로 boardVO 전달.
//			return "board/boardwrite";
//		}
	...
	public String doBoardModify(Model model, @PathVariable int id, BoardVO boardVO, @RequestParam MultipartFile file, 
			@SessionAttribute("_LOGIN_USER_") MemberVO memberVO) {
...
//		boardVO.setEmail(memberVO.getEmail());
	...
	public ResponseEntity<Resource> downloadFile(@PathVariable int id) {
		...
		BoardVO boardVO = this.boardService.getOneBoard(id, false);
		// 만약, 게시글이 존재하지 않는다면 "잘못된 접근입니다." 라는 에러를 사용자에게 보여준다.
		if (boardVO == null) {
			throw new PageNotFoundException();
		}
		...
		// 첨부된 파일이 있을 경우엔 파일을 사용자에게 보내준다. (Downlaod)
		// 사용자가 업로드한 파일 원본이름 // DB에 저장된 난수화된 파일이름
	...
	public ResponseEntity<Resource> downloadExcelFile() {
		...
		// 엑셀 파일 만들기
		File storedFile = fileHandler.getStroedFile("게시글_목록.xlsx");
